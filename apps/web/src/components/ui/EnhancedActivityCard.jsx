...
const EnhancedActivityCard = ({ activity, index = 0, onJoin, onLike }) => {
  ...
  // Support both id and _id for activity identifier
  ...
  const handleLike = async e => {
    ...
    try {
      await apiService.toggleLike(activityId, 'activity');
      onLike?.(activityId, newLikedState);
    } catch (error) {
      // Revert on error
      setIsLiked(!newLikedState);
  ...
  const handleJoin = async e => {
    ...
    try {
      if (newJoinedState) {
        await apiService.joinEvent(activityId);
      } else {
        await apiService.leaveEvent(activityId);
      }
      onJoin?.(activityId, newJoinedState);
    } catch (error) {
      // Revert on error
      setIsJoined(!newJoinedState);
  ...
  const animateJoinButton = joined => {
    ...
    if (joined) {
      tl.to(button, {
        ...
        button.querySelector('.success-icon'),
        {
          duration: 0.6,
          scale: 0,
          rotation: 180,
  ...
  const formatCurrency = (amount) => {
    if (!amount) return t('activities.card.free', 'Free');
        ...
        {activity.capacity && (
          <div className="mb-4">
            <div className="flex justify-between text-sm text-gray-600 mb-1">
              <span>{t('activities.card.availableSeats', 'Available Seats')}</span>
              <span>
                {activity.capacity - (activity.attendees || 0)}{' '}
                {t('activities.card.remaining', 'remaining')}
              </span>
            </div>
          ...
          >
            {isLoading ? (
              <div className="flex items-center">
                ...
                {t('activities.card.loading', 'Loading...')}
              </div>
            ) : isJoined ? (
              <div className="flex items-center">
                <svg
                  className="success-icon w-4 h-4 mr-2"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                ...
                {t('activities.card.joined', 'Joined')}
              </div>
            ) : (
              t('activities.card.joinNow', 'Join Now')
            )}
          </button>
        </div>
      </div>
