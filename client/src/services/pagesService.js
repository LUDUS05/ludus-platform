...
const pagesService = {
  ...
  getPagesByPlacement: async (placement) => {
    try {
      const response = await api.get(`/api/pages/by-placement/${placement}`);
  ...
  getPagesForMenu: async (placement) => {
    try {
      const response = await api.get(`/api/pages/menu/${placement}`);
      return response.data;
    } catch (error) {
  ...
  getPageByUrl: async (url) => {
    try {
      const response = await api.get(`/api/pages/by-url${url}`);
      return response.data;
    } catch (error) {
